...
- Keep the RL frozen-policy selected-point result as the center.
- Keep target-path quantities diagnostic only.
- Keep CC-TA-LBIP auxiliary.
- Include the same-forecast table in the main paper only if the analysis note says it is strong enough.
- Do not widen the claim.
- Do not turn this into a full-paper rewrite.
...
- RL main result is clearly central
- accounting gap and friction curve support the same story
- CC-TA-LBIP remains auxiliary
- the writing reads like a focused workshop paper, not a compressed full paper
