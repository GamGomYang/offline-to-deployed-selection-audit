...
\begin{table*}[t]
\caption{Deployed-selection robustness report card. Each row reports one domain $\times$ fixed interface $\times$ friction cell and records whether the forecast-side winner remains the deployed-side winner after the same fixed interface is applied. Subopt. reports the seed-level deployed-suboptimal share/count. The 95\% CI column reports the bootstrap CI for the mean deployed gap, not the binomial CI for the suboptimal share.}
