...
DEFAULT_OUTPUT_NOTE = (
    ...
    / "generalization"
    / "notes"
    / "arch_vol_scaled_eta_validation_note.md"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate validation-only volatility-scaled eta results.")
...
def _format_alpha_key(value: float) -> str:
    return f"{float(value):.3f}"


def _format_eta_min_key(value: float) -> str:
    return f"{float(value):.2f}"


def _config_key(alpha: float, eta_min: float, lookback: int) -> str:
    return f"alpha_{_format_alpha_key(alpha)}__etaMin_{_format_eta_min_key(eta_min)}__lb_{int(lookback)}"


def _load_rows(raw_root: Path) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    for result_path in sorted(raw_root.glob("kappa_*/alpha_*__etaMin_*/seed_*/result.json")):
        rows.append(json.loads(result_path.read_text()))
    if not rows:
        raise FileNotFoundError(f"No vol-scaled validation result.json files found under {raw_root}")
...
def _build_row_metrics(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in raw_df.itertuples(index=False):
        ...
        out_rows.append(
            {
                "config_key": _config_key(float(row.alpha), float(row.eta_min), int(row.lookback)),
    ...
    return out_df.sort_values(["alpha", "eta_min", "lookback", "kappa"], key=lambda s: s.map(kappa_sort_key) if s.name == "kappa" else s).reset_index(drop=True)


def _attach_config_summary(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    config_rows: list[dict[str, object]] = []
    for (alpha, eta_min, lookback), group in df.groupby(["alpha", "eta_min", "lookback"], sort=True):
        ...
        config_rows.append(
            {
                "config_key": _config_key(float(alpha), float(eta_min), int(lookback)),
                "alpha": float(alpha),
    ...
    out_df = df.merge(
        config_df,
        on=["config_key", "alpha", "eta_min", "lookback"],
        how="left",
    )
...
def _write_note(config_df: pd.DataFrame, output_note: Path) -> None:
    ...
    if not eligible_list:
        eligible_list.append("- No volatility-scaled configuration met the current validation eligibility rule.")
    ...
    if champion is None and not config_df.empty:
        ...
        champion_text = (
            ...
            f"is provisionally `{best_overall.config_key}`, but it remains ineligible under the shared deadband-style validation rule."
        )
        ...
        champion_text = (
            "The recommended volatility-scaled configuration for later test evaluation is "
            f"`{champion.config_key}`. It is selected by largest mean positive-cost `ΔSharpe_exec`, "
            "then largest summed disagreement strength, then largest mean positive-cost turnover reduction, "
            "and finally by the simpler parameter preference that favors larger `eta_min` and then larger `alpha`."
        )

    text = f"""# Volatility-Scaled Eta Validation Note
