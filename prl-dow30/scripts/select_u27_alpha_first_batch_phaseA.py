...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Select Phase A alpha-first challengers for Phase B.")
    parser.add_argument(
        "--summary-csv",
        type=str,
        default="",
    ...
    parser.add_argument(
        "--summary-glob",
        type=str,
        default="outputs/reports/u27_alpha_first_batch_phaseA_summary_*.csv",
...
def _selection_warning(row: dict[str, Any]) -> list[str]:
    ...
    if pd.notna(k001_delta) and float(k001_delta) <= 0.0 and not soft_pass:
        warnings.append("k001_mean_delta_nonpositive_without_soft_pass")
    return warnings
...
def main() -> None:
    ...
    out_json = _resolve(args.out_json) if args.out_json else ROOT / "outputs" / "reports" / f"u27_alpha_first_batch_phaseA_decision_{ts}.json"
    out_md = _resolve(args.out_md) if args.out_md else ROOT / "outputs" / "reports" / f"u27_alpha_first_batch_phaseA_decision_{ts}.md"
    ...
    lines: list[str] = []
    lines.append("# U27 Alpha First Batch Phase A Decision")
    lines.append("")
    ...
    if selected_records:
        for item in selected_records:
            lines.append(
                ...
                f"k001_mean_delta_sharpe={item['k001_mean_delta_sharpe']} warnings={item['warnings']}"
            )
    else:
