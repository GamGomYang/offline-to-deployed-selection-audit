...
if [[ -z "$MODEL_ROOT" || ! -d "$MODEL_ROOT" ]]; then
  echo "[ERROR] Missing winner model root for post-adoption validation: ${MODEL_ROOT:-<unset>}"
  exit 1
fi

echo "[INFO] alpha_postadopt winner_tag_20k=${WINNER_TAG_20K:-unset}"
echo "[INFO] alpha_postadopt winner_tag_100k=${WINNER_TAG_100K:-unset}"
echo "[INFO] alpha_postadopt current_config=${CURRENT_CONFIG}"
echo "[INFO] alpha_postadopt model_root=${MODEL_ROOT}"
echo "[INFO] alpha_postadopt forward_config=${FORWARD_CONFIG}"
echo "[INFO] alpha_postadopt operational_config=${OPERATIONAL_CONFIG}"
...
env \
  ...
  bash scripts/run_u27_eta082_post_adoption.sh
