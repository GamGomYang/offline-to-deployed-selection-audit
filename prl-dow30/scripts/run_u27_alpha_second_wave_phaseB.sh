...
BASELINE_TAG="${BASELINE_TAG:-u27_eta082_alpha2_ctrl_20k_r1}"
...
if [[ -z "$PHASEB_CANDIDATES_RAW" && "$AUTO_SELECT" == "1" ]]; then
  SELECTOR_CMD=(
    "$PYTHON_BIN" scripts/select_u27_alpha_second_wave_phaseA.py
...
echo "[INFO] alpha_second_wave_phaseB baseline_tag=${BASELINE_TAG}"
echo "[INFO] alpha_second_wave_phaseB candidates=${PHASEB_CANDIDATES_RAW}"
if [[ -n "$PHASEA_SUMMARY_CSV" ]]; then
  echo "[INFO] alpha_second_wave_phaseB phaseA_summary_csv=${PHASEA_SUMMARY_CSV}"
fi
