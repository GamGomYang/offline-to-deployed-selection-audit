...
DEFAULT_TAUS = (0.0, 0.03, 0.1, 0.3, 1.0)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Turnover-Aware Linear Information-Parity Baseline (TA-LBIP).")
    ...
    parser.add_argument("--taus", nargs="+", type=float, default=list(DEFAULT_TAUS), help="Tau grid.")
    parser.add_argument("--offline", action="store_true", help="Use cache only / offline mode.")
    parser.add_argument("--skip-selection", action="store_true", help="Skip validation tau selection artifact.")
...
def _resolve_base_config(cfg: dict[str, Any]) -> LBIPConfig:
    env_cfg = cfg.get("env", {}) or {}
    base_cfg = cfg.get("talibp", {}) or {}
    return LBIPConfig(
...
def _write_aggregate(metrics_df: pd.DataFrame, out_root: Path) -> pd.DataFrame:
    grouped = (
        metrics_df.groupby(["kappa", "tau"], as_index=False)
        .agg(
...
def _write_selection(aggregate_df: pd.DataFrame, out_root: Path, kappas: list[float]) -> dict[str, Any]:
    ...
    rows = []
    taus = sorted(pd.unique(aggregate_df["tau"]).tolist())
    baseline_tau = 0.0 if 0.0 in taus else float(taus[0])
    for tau in taus:
        row = {"tau": float(tau)}
        score_vals = []
        for kappa in pos_kappas:
            cur = aggregate_df[(aggregate_df["tau"] == tau) & (aggregate_df["kappa"] == kappa)].iloc[0]
            base = aggregate_df[(aggregate_df["tau"] == baseline_tau) & (aggregate_df["kappa"] == kappa)].iloc[0]
            score_vals.append(float(cur["median_sharpe"]))
            row[f"sharpe_kappa_{kappa:g}"] = float(cur["median_sharpe"])
            row[f"delta_sharpe_vs_tau0_kappa_{kappa:g}"] = float(cur["median_sharpe"] - base["median_sharpe"])
        ...
        row["kappa0_sharpe"] = float(aggregate_df[(aggregate_df["tau"] == tau) & (aggregate_df["kappa"] == 0.0)].iloc[0]["median_sharpe"]) if (aggregate_df["kappa"] == 0.0).any() else float("nan")
        rows.append(row)
    ...
    payload = {
        "selection_rule": "smallest positive tau within 95% of best positive-cost validation score",
    ...
    (sel_dir / "validation_tau_selection.json").write_text(json.dumps(payload, indent=2))
    rows_df.to_csv(sel_dir / "validation_tau_selection.csv", index=False)
    return payload


def _write_selected_vs_tau0(aggregate_df: pd.DataFrame, selected_tau: float, out_root: Path) -> None:
    rows = []
    sel = aggregate_df[aggregate_df["tau"] == selected_tau].set_index("kappa")
    base = aggregate_df[aggregate_df["tau"] == 0.0].set_index("kappa")
    for kappa in sorted(pd.unique(aggregate_df["kappa"]).tolist()):
        rows.append({
            ...
            "baseline_tau0_sharpe": float(base.loc[kappa, "median_sharpe"]),
            ...
            "baseline_tau0_turnover": float(base.loc[kappa, "median_turnover_exec"]),
        })
    pd.DataFrame(rows).to_csv(out_root / "selected_vs_tau0.csv", index=False)
...
def main() -> None:
    ...
    base_config = _resolve_base_config(cfg)
    experiment_name = str(output_cfg.get("experiment_name", config_path.stem))
    kappas = [float(v) for v in args.kappas]
    taus = [float(v) for v in args.taus]

    metrics_rows: list[dict[str, Any]] = []
    fit_root = out_root / "fit"
    fit_root.mkdir(parents=True, exist_ok=True)
    (fit_root / "signal_spec_seed0.json").write_text(json.dumps(signal_spec, indent=2))

    for tau in taus:
        tau_cfg = LBIPConfig(**{**base_config.__dict__, "anchor_strength": float(tau)})
        ...
        tau_tag = f"tau_{float(tau):g}"
        tau_fit_dir = fit_root / tau_tag
        save_lbip_model(model, tau_fit_dir / "talibp_model_seed0.npz")
        summary = fit_summary_dict(model)
        summary.update({
            "target_mode": str(tau_cfg.target_mode),
            "tau": float(tau),
            "equal_weight_shrink": float(tau_cfg.equal_weight_shrink),
            "training_eta": float(tau_cfg.training_eta),
        })
        (tau_fit_dir / "talibp_summary_seed0.json").write_text(json.dumps(summary, indent=2))
        run_id = f"{experiment_name}_talibp_tau{tau:g}_seed0"
        for kappa in kappas:
            arm_dir = tau_tag
            seed_dir = out_root / f"kappa_{float(kappa):g}" / arm_dir / "seed_0"
            seed_dir.mkdir(parents=True, exist_ok=True)
            eval_id = f"{run_id}__kappa{kappa:.6g}"
    ...
    metrics_df = pd.DataFrame(metrics_rows).sort_values(["kappa", "tau"]).reset_index(drop=True)
    ...
    if not args.skip_selection:
        ...
        selected_tau = float(payload["selected_tau"])
        _write_selected_vs_tau0(aggregate_df, selected_tau, out_root)
    ...
    protocol = {
        ...
        "kappas": kappas,
        "taus": taus,
        "baseline_name": "Turnover-Aware Linear Information-Parity Baseline",
        "baseline_short_name": "TA-LBIP",
        "fit_base_config": {
            "ridge_alpha": float(base_config.ridge_alpha),
            "fit_passes": int(base_config.fit_passes),
            "mean_variance_risk_aversion": float(base_config.mean_variance_risk_aversion),
            "equal_weight_shrink": float(base_config.equal_weight_shrink),
            "training_eta": float(base_config.training_eta),
        },
        "selected_tau": selected_tau,
    ...
    if selected_tau is not None:
        print(f"SELECTED_TAU={selected_tau}")
        print(f"WROTE_SELECTION={out_root / 'selection' / 'validation_tau_selection.json'}")
        print(f"WROTE_SELECTED_SUMMARY={out_root / 'selected_vs_tau0.csv'}")
