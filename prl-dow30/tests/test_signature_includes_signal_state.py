...
def test_signature_and_metadata_include_signal_state(tmp_path, monkeypatch):
    ...
    def _fake_prepare_market_and_features(*args, **kwargs):
        ...
        ramp = np.linspace(-0.0003, 0.0003, len(dates))
    ...
    cfg = {
        "mode": "smoke",
        "config_path": "configs/prl_100k_signals.yaml",
        "dates": {
    ...
    assert meta["feature_flags"]["signal_state"] is True
    assert meta["feature_flags"]["signal_names"] == ["reversal_5d", "short_term_reversal"]
    assert meta["obs_dim_expected"] == 12
    assert meta["signals"]["enabled"] is True

    expected_signature = compute_env_signature(
        asset_list,
        2,
        2,
        feature_flags=meta["feature_flags"],
        cost_params={"transaction_cost": 0.0, "risk_lambda": 0.0, "rebalance_eta": None},
        schema_version="v1",
    )
    assert meta["env_signature_hash"] == expected_signature
