...
def test_multi_window_eval_writes_eval_window_column(tmp_path, monkeypatch):
    ...
    cfg = {
        ...
        "eval": {
            "windows": [
                {"name": "W1", "start": "2024-01-01", "end": "2025-12-30"},
                {"name": "W2", "start": "2022-02-15", "end": "2023-12-29"},
            ]
        },
    ...
    dates = pd.date_range("2022-01-01", "2025-12-30", freq="B")
    ...
    assert "eval_window" in metrics_df.columns
    assert set(metrics_df["eval_window"]) == {"W1", "W2"}

    regime_df = pd.read_csv(regime_path)
    assert "eval_window" in regime_df.columns
    assert set(regime_df["eval_window"]) == {"W1", "W2"}

    assert Path("outputs/reports/trace_runid_prl_seed0_W1.parquet").exists()
    assert Path("outputs/reports/trace_runid_prl_seed0_W2.parquet").exists()
