...
def test_trace_contains_eval_id_run_id(tmp_path, monkeypatch):
    ...
    assert required.issubset(trace_df.columns)
    assert trace_df["eval_id"].notna().all()
    assert trace_df["run_id"].notna().all()
